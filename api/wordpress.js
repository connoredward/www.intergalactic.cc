...
async function getWordpressData({ pSlug, pTags, pageNumber }) {
  return new Promise(async (res, rej) => {
    const categories = await fetch(
      wordpressUrl + `categories?slug=${pSlug}&per_page=100&_embed=1`
    ).then((res) => res.json());
    const tags = await fetch(wordpressUrl + `tags?slug=${pTags}&per_page=100&_embed=1`).then(
      (res) => res.json()
    );
    const posts = await fetch(
      wordpressUrl +
        `posts?${
          !tags[0] ? `categories=${categories[0].id}` : `tags=${tags[0].id}`
        }&page=${pageNumber}&per_page=10&_embed=1`
    ).then((res) => res.json());
...
export async function getVimeoModalUrl(slug) {
  const f = await fetch(wordpressUrl + `posts?slug=${slug}`).then((res) => res.json());
  return {
...
async function photoUrl(id) {
  return new Promise(async (res, rej) => {
    const url = await fetch(
      `https://intergalacticcms.wpcomstaging.com/wp-json/wp/v2/media/${id}`
    ).then((res) => res.json());
...
export async function getPhotos(pSlug) {
  const post = await fetch(wordpressUrl + `posts?slug=${pSlug}`).then((res) => res.json());
  return await Promise.all(post[0]?.x_metadata.gallery.split(',').map((i) => photoUrl(i)));
}
