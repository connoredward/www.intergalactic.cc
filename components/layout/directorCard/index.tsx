...
export function DirectorCard(props) {
  const { videoSrc, imgSrc, children, className, onClick, gridRow, gridColumn, showContent } =
    props;
    ...
    >
      <div
        className={classNames(
          styles['card_content'],
          styles[showContent ? 'show_content' : undefined]
        )}
      >
        {children}
      </div>
      {videoSrc && (
        <video className={styles[videoSize]} ref={videoRef} src={videoSrc} autoPlay muted loop />
      )}
    </div>
  );
}
